...
class VendaEditarScreen extends StatefulWidget {
  ...
  const VendaEditarScreen({super.key, required this.id});

  @override
  VendaEditarState createState() => VendaEditarState();
}

class VendaEditarState extends State<VendaEditarScreen> {
  String? jwt;
  String? role;
  String? idUsuario;
  ...
  Future<void> _confirmarVenda() async {
    ...
    try {
      ...
      final response = await Dio().patch(
        'http://localhost:3300/vendas/update',
        data: vendaData,
        options: options,
      );
